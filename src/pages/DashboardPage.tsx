...
import { BookOpen, Trophy, Clock, ChevronRight, PlayCircle, BarChart3, AlertCircle, Star, ArrowRight } from "lucide-react";
        ...
        const fetchData = async () => {
            try {
                ...
                if (user) {
                    ...
                    setTopUsers(top.slice(0, 3));
                    
                    const myRank = top.findIndex((u: any) => u.uid === user.uid) + 1;
                    if (myRank > 0) setUserRank(myRank);
                }
            } catch (error) {
                ...
                <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
                    <header className="mb-10">
                        ...
                        >
                            ...
                            <p className="text-slate-500 mt-1">Welcome back, {user?.name || user?.email?.split('@')[0]}</p>
                        </motion.div>
                        ...
                        {!loading && userRank && (
                            <motion.div
                                ...
                                className="flex items-center gap-4 bg-white px-6 py-3 rounded-2xl border-2 border-indigo-100 shadow-lg shadow-indigo-50"
                            >
                                <div className="h-10 w-10 bg-indigo-600 rounded-xl flex items-center justify-center shadow-lg shadow-indigo-200">
                                    <Trophy className="h-5 w-5 text-white" />
                                </div>
                                <div>
                                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-none mb-1">Global Rank</p>
                                    <p className="text-xl font-black text-slate-900 leading-none">#{userRank}</p>
                                </div>
                            </motion.div>
                        )}
                    </header>
                        ...
                        <div className="space-y-8">
                                ...
                                <div className="bg-gradient-to-br from-indigo-500 to-purple-600 rounded-3xl p-1 relative overflow-hidden shadow-xl shadow-indigo-200">
                                    ...
                                    <div className="bg-white/10 backdrop-blur-md rounded-[28px] p-6 border border-white/20">
                                        ...
                                        <div className="space-y-3 relative z-10">
                                            ...
                                            {topUsers.length > 0 && (
                                                <Link to="/leaderboard" className="mt-4 flex items-center justify-center gap-2 py-3 bg-white/10 text-white rounded-xl text-sm font-bold hover:bg-white/20 transition-all border border-white/10">
                            ...
                            <section>
                                ...
                                <div className="space-y-4">
                                    ...
                                    {recentResults.length > 0 && (
                                        <Link to="/results" className="block text-center text-sm font-semibold text-indigo-600 hover:text-indigo-700 py-2">
                                            View all results
                                        </Link>
                                    )}
                                </div>
